...
from  rl_algorithms.sac_qmix import SACQMIXAgent
import torch
from  rl_algorithms.utilities import soft_update, transpose_to_tensor, transpose_list, gumbel_softmax
import numpy as np
...
class MASACQMIX:
    def __init__(self, num_agents = 3, num_landmarks = 1, landmark_depth=15., discount_factor=0.95, tau=0.02, lr_actor=1.0e-2, lr_critic=1.0e-2, weight_decay=1.0e-5, device = 'cpu', rnn = True, alpha = 0.2, automatic_entropy_tuning = True, dim_1=64, dim_2=32):
        super(MASACQMIX, self).__init__()
        
        # in_actor = 1*2*2 + num_landmarks*2 + (num_agents-1)*2 + num_landmarks + 1 #all previous tests
        # in_actor = 1*2*2 + num_landmarks*2 + (num_agents-1)*2 + num_landmarks + 1 + 1#test with target depth for science
        ...
        if DISCRETE_ACTIONS == False:
            ...
            in_actor = 1*2*2 + num_landmarks*2 + (num_agents-1)*2 + num_landmarks + 1*num_landmarks + 2 +3#test with target depth and agent's origin for science
        # in_actor = 1*2*2 + (num_agents-1)*2 + num_landmarks + 1 #test without predicted target position on observation state
        ...
        self.masac_agent = [SACQMIXAgent(in_actor, hidden_in_actor, hidden_out_actor, out_actor, in_critic, hidden_in_critic, hidden_out_critic, rnn_num_layers, rnn_hidden_size_actor, rnn_hidden_size_critic, lr_actor=lr_actor, lr_critic=lr_critic, weight_decay=weight_decay, device=device, rnn = rnn, alpha = alpha, automatic_entropy_tuning=automatic_entropy_tuning) for _ in range(num_agents)]
